#include <stdio.h>
#include <stdlib.h>
#include <time.h>

int Read_Number()
{
	int Number = 0;

	do
	{
		printf("Pleas enter the Number ?\n");
		scanf("%d", &Number);

	}while(Number <= 0);
	return (Number);
}
int Random_Number(int From, int To)
{
	int RandNum = rand() % (To - From + 1) + From;
	return (RandNum);
}
int *Fill_Array_With_Random_Number(int lenght)
{
	int *Arr = (int *)malloc(sizeof(int) * lenght + 1);
	
	if(Arr == NULL)
	{
		return (NULL);
	}

	for(int i = 0; i < lenght; i++)
	{
		Arr[i] = Random_Number(1, 100);
	}
	return (Arr);
}
void Add_Array_Element(int Number,int *Arr2, int Count)
{
	Count++;
	Arr2[Count - 1] = Number;
}
int *Copy_Array_To_Another_Array(int *Arr, int *Arr2, int lenght)
{
	for(int i = 0; i < lenght; i++)
	{
		Add_Array_Element(Arr[i], Arr2, i);	
	}
	return (Arr2);
}
void Print_Array(int *Arr, int lenght)
{
	for(int i = 0; i < lenght; i++)
	{
		printf("%d ", Arr[i]);
	}
	putchar('\n');
}
int main()
{
	srand((unsigned) time(NULL));

	int lenght = Read_Number();
	int *Arr = Fill_Array_With_Random_Number(lenght);
	int *Arr2 = (int *)malloc(sizeof(int) * (lenght + 1));

	Copy_Array_To_Another_Array(Arr, Arr2, lenght);

	printf("Array Befor Copy Element : ");
	Print_Array(Arr, lenght);

	printf("Array After Copy elemnt : ");
	Print_Array(Arr2, lenght);

	free(Arr);
	free(Arr2);
	return (0);
}
